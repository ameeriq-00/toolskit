const strings = {
  // App General
  appTitle: "تطبيق أدوات متعددة",
  welcomeMessage: "مرحبًا بكم في تطبيق أدوات متعددة",
  description:
    "يوفر هذا التطبيق أدوات متعددة عبر الإنترنت لمساعدتك في مهام مختلفة. استخدم الشريط الجانبي للتنقل بين الأدوات المختلفة.",
  login: "تسجيل الدخول",
  analyze: "تحليل",
  noDataAvailable: "لا توجد بيانات متاحة",

  // Excel Analysis Common
  filteredCalls: "الجسور",
  aggregatedCallerNumbers: "أرقام المتصلين المجمعة",
  imeiUsage: "استخدام IMEI",
  mostVisitedSites: "أكثر المواقع زيارة",
  analysisResult: "نتيجة التحليل:",

  // Site Map
  siteMap: "خريطة المواقع",
  noSiteDataAvailable: "لا توجد بيانات موقع متاحة",
  noValidCoordinates: "لا توجد إحداثيات صالحة للعرض على الخريطة",
  siteId: "معرف البرج",
  numberOfVisits: "عدد الزيارات",
  latitude: "خط العرض",
  longitude: "خط الطول",
  sortAscending: "ترتيب تصاعدي",
  sortDescending: "ترتيب تنازلي",
  sitesAtLocation: "المواقع في هذا الموقع",
  totalVisits: "اجمالي التحركات ",
  fullScreenMapButton: "خريطة ملء الشاشة",
  exitFullScreenButton: "الخروج من وضع ملء الشاشة",

  // User Authentication
  username: "اسم المستخدم",
  password: "كلمة المرور",

  // Number Lookup
  numberLookup: "البحث عن الرقم",
  enterNumber: "أدخل الرقم",
  lookup: "بحث",
  lookupResult: "نتيجة البحث",

  // Excel Analyzer Template Selection
  excelAnalyzerTitle: "تحليل ملفات إكسل",
  uploadSiteInfoTitle: "رفع معلومات المواقع",
  uploadSiteInfoButton: "رفع معلومات المواقع",
  templateSelectLabel: "اختر نوع القالب",
  originalTemplate: "القالب الأصلي",
  newTemplate: "القالب الجديد",

  // File Upload
  mainFileLabel: "الملف الرئيسي",
  imeiFileLabel: "ملف IMEI",
  analyzeButton: "تحليل",
  selectSiteInfoFile: "الرجاء اختيار ملف معلومات المواقع أولاً",
  siteInfoUploadError: "خطأ في رفع معلومات المواقع. الرجاء المحاولة مرة أخرى",
  uploadFile: "رفع الملف",
  fileUploaded: "تم رفع الملف بنجاح",
  uploadError: "خطأ في رفع الملف",

  // Excel Analyzer Z Tool
  excelAnalyzerZTitle: "تحليل ملفات إكسل Z",
  uploadMainFile: "رفع الملف الرئيسي",
  uploadImeiFile: "رفع ملف IMEI",
  processFiles: "تحليل الملفات",
  bothFilesRequired: "يجب رفع الملفين",
  processingFiles: "جاري تحليل الملفات...",
  analysisComplete: "تم اكتمال التحليل",
  analysisError: "حدث خطأ أثناء التحليل",

  // Site Information
  siteName: "اسم البرج",
  siteLocation: "موقع البرج",
  siteVisits: "عدد الزيارات للموقع",
  siteDetails: "تفاصيل الموقع",
  siteCoordinates: "إحداثيات البرج",

  // Error Messages
  errorNoData: "لا توجد بيانات",
  errorInvalidFile: "ملف غير صالح",
  errorProcessing: "خطأ في معالجة البيانات",
  errorUpload: "خطأ في رفع الملف",
  errorInvalidCoordinates: "إحداثيات غير صالحة",

  // Success Messages
  successUpload: "تم رفع الملف بنجاح",
  successAnalysis: "تم تحليل البيانات بنجاح",
  successSiteInfo: "تم حفظ معلومات المواقع بنجاح",

  // Time Analysis
  hourlyDistribution: "التوزيع حسب الساعة",
  dailyPattern: "النمط اليومي",
  weeklyPattern: "النمط الأسبوعي",
  peakHours: "ساعات الذروة",
  peakDays: "أيام الذروة",
  numberOfCalls: "عدد المكالمات",
  hour: "الساعة",
  calls: "مكالمات",
  timeAnalysis: "تحليل الوقت",

  hourDetails: "تفاصيل النشاط بالساعة",
  dailyDetails: "تفاصيل النشاط اليومي",
  weeklyDetails: "تفاصيل النشاط الأسبوعي",
  periodDetails: "تفاصيل الفترات",
  clickForDetails: "انقر للمزيد من التفاصيل",
  numberOfActivities: "عدد النشاطات",
  timeDistribution: "توزيع النشاط",
  periodDistribution: "توزيع الفترات",
  detailsFor: "التفاصيل لـ",
  closeDetails: "إغلاق",
  timeDetails: "تفاصيل الوقت",
  close: "إغلاق",

  // Movement Map Strings
  movementAnalysis: "تحليل التنقلات",
  movementMap: "خريطة التنقلات",
  noMovementData: "لا توجد بيانات تنقل متاحة",
  movementStats: "إحصائيات التنقل",
  movementPatterns: "أنماط التنقل",

  // Distance and Measurements
  totalDistance: "المسافة الإجمالية",
  averageDistance: "متوسط المسافة",
  maxDistance: "أقصى مسافة",
  totalMovements: "عدد التنقلات",
  km: "كم",

  // Time and Duration
  timestamp: "التوقيت",
  timeToNext: "الوقت حتى التنقل التالي",
  duration: "المدة",
  startTime: "وقت البداية",
  endTime: "وقت النهاية",
  hours: "ساعات",

  // Map Controls
  showAllPaths: "إظهار جميع المسارات",
  hideAllPaths: "إخفاء جميع المسارات",
  showPath: "إظهار المسار",
  hidePath: "إخفاء المسار",
  zoomIn: "تكبير",
  zoomOut: "تصغير",

  // Locations
  fromLocation: "من موقع",
  toLocation: "إلى موقع",
  currentLocation: "الموقع الحالي",
  nextLocation: "الموقع التالي",
  finalDestination: "الوجهة النهائية",
  currentPosition: "الموقع الحالي",

  // Movement Details
  movementDetails: "تفاصيل التنقل",
  movementTime: "وقت التنقل",
  movementDistance: "مسافة التنقل",
  movementDuration: "مدة التنقل",

  // Map View Controls
  fullScreenView: "عرض ملء الشاشة",
  normalView: "العرض العادي",
  resetView: "إعادة ضبط العرض",

  // Site Information
  siteDetails: "تفاصيل الموقع",
  siteName: "اسم البرج",
  siteLocation: "موقع البرج",
  visitCount: "عدد الزيارات",

  // Time Periods
  morningPeriod: "فترة الصباح",
  afternoonPeriod: "فترة الظهر",
  eveningPeriod: "فترة المساء",
  nightPeriod: "فترة الليل",

  // Movement Types
  shortMovements: "تنقلات قصيرة",
  mediumMovements: "تنقلات متوسطة",
  longMovements: "تنقلات طويلة",

  // Statistics and Analysis
  averageSpeed: "متوسط السرعة",
  totalTime: "الوقت الإجمالي",
  mostFrequentPath: "المسار الأكثر تكراراً",
  peakMovementTimes: "أوقات الذروة للتنقلات",

  // Status Messages
  loadingMovements: "جاري تحميل بيانات التنقلات...",
  noPathSelected: "لم يتم اختيار مسار",
  noDataForPeriod: "لا توجد بيانات لهذه الفترة",

  // Movement Timeline
  timeline: "الجدول الزمني",
  timelineStart: "بداية الفترة",
  timelineEnd: "نهاية الفترة",
  timelineControls: "التحكم بالجدول الزمني",

  // Interactive Controls
  play: "تشغيل",
  pause: "إيقاف",
  next: "التالي",
  previous: "السابق",
  speed: "السرعة",

  // Buttons
  applyFilter: "تطبيق التصفية",
  clearFilter: "مسح التصفية",
  updateView: "تحديث العرض",
  closeView: "إغلاق العرض",

  // Map Legends
  startPoint: "نقطة البداية",
  endPoint: "نقطة النهاية",
  intermediatePoint: "نقطة وسيطة",
  activePath: "المسار النشط",
  inactivePath: "المسار غير النشط",

  // Analysis Categories
  dailyMovements: "التنقلات اليومية",
  weeklyMovements: "التنقلات الأسبوعية",
  monthlyMovements: "التنقلات الشهرية",

  // Filter Options
  filterByDistance: "تصفية حسب المسافة",
  filterByTime: "تصفية حسب الوقت",
  filterByLocation: "تصفية حسب الموقع",

  callPatterns: "أنماط المكالمات",
  dailyDistribution: "التوزيع اليومي",
  weeklyDistribution: "التوزيع الأسبوعي",
  reciprocalPatterns: "أنماط المكالمات المتبادلة",
  contactNumber: "رقم الاتصال",
  callsMade: "المكالمات الصادرة",
  callsReceived: "المكالمات الواردة",
  totalCalls: "إجمالي المكالمات",
  callRatio: "نسبة المكالمات",
  morning: "صباحاً",
  afternoon: "ظهراً",
  evening: "مساءً",
  night: "ليلاً",

  imeiUsageTimeline: "مخطط استخدام IMEI",
  imeiUsageDistribution: "توزيع استخدام IMEI",
  imeiUsageDetails: "تفاصيل استخدام IMEI",
  imeiNumber: "رقم IMEI",
  usageCount: "عدد مرات الاستخدام",
  usagePeriod: "فترة الاستخدام",

  // Behavioral Analysis
  routineSummary: "ملخص الروتين",
  typicalStartEnd: "ساعات النشاط المعتادة",
  mostActiveDay: "اليوم الأكثر نشاطاً",
  peakActivityHour: "ساعة الذروة",
  detectedAnomalies: "الأنماط غير العادية",
  unusualTiming: "توقيت غير معتاد",
  unusualLocation: "موقع غير معتاد",
  highActivity: "نشاط مرتفع",
  lowActivity: "نشاط منخفض",
  noAnomaliesDetected: "لم يتم اكتشاف أنماط غير عادية",
  hourlyActivityPattern: "نمط النشاط خلال اليوم",
  weeklyPattern: "النمط الأسبوعي",
  locationDistribution: "توزيع المواقع",
  activityStatistics: "إحصائيات النشاط",
  averageDailyCalls: "متوسط المكالمات اليومية",
  totalDays: "إجمالي الأيام",
  activeDays: "أيام النشاط",
  quietHours: "ساعات الهدوء",
  calls: "المكالمات",

  // Enhanced Time Analysis
  statisticalSummary: "ملخص إحصائي",
  totalCalls: "إجمالي المكالمات",
  averageCallsPerDay: "متوسط المكالمات اليومية",
  activeHoursPerDay: "ساعات النشاط اليومية",
  patternConsistency: "ثبات النمط",
  hourlyDistribution: "التوزيع حسب الساعة",
  dayPeriodDistribution: "توزيع فترات اليوم",
  weekendVsWeekday: "مقارنة عطلة نهاية الأسبوع مع أيام الأسبوع",
  averageWeekdayCalls: "متوسط مكالمات أيام الأسبوع",
  averageWeekendCalls: "متوسط مكالمات نهاية الأسبوع",
  activitySpikes: "فترات النشاط المرتفع",
  noActivitySpikes: "لا توجد فترات نشاط مرتفع",
  peakHours: "ساعات الذروة",
  calls: "مكالمات",
  morning: "صباحاً",
  afternoon: "ظهراً",
  evening: "مساءً",
  night: "ليلاً",

  // Movement Map
  dailyView: "عرض يومي",
  weeklyView: "عرض أسبوعي",
  uniqueLocations: "المواقع الفريدة",
  currentPeriod: "الفترة الحالية",
  playMovements: "تشغيل التنقلات",
  pauseMovements: "إيقاف مؤقت",
  nextPeriod: "الفترة التالية",
  previousPeriod: "الفترة السابقة",
  dailyStats: "إحصائيات يومية",
  weeklyStats: "إحصائيات أسبوعية",

  // Site Analysis
  visitsByCity: "النسبة حسب المدن",
  cityComparison: "مقارنة المدن",
  allSites: "جميع المواقع",
  byCity: "حسب المدينة",
  selectCity: "اختر المدينة",
  cities: "المدن",
  totalSites: "إجمالي المواقع",
  detailedSiteInfo: "تفاصيل المواقع",
  mostActiveCity: "المدينة الأكثر نشاطاً",

  // Data Explorer General
  dataExplorer: "استكشاف البيانات",
  filterData: "تصفية البيانات",
  clearFilters: "مسح التصفية",
  fullScreenView: "عرض ملء الشاشة",
  exitFullScreen: "الخروج من ملء الشاشة",

  // Statistics Cards
  totalRecords: "إجمالي السجلات",
  uniqueCallers: "المتصلين الفريدين",
  uniqueLocations: "المواقع الفريدة",
  avgDuration: "متوسط المدة",

  // Filters
  startDate: "تاريخ البداية",
  endDate: "تاريخ النهاية",
  location: "الموقع",
  callType: "نوع المكالمة",
  timeOfDay: "وقت اليوم",
  durationRange: "نطاق المدة",
  numberPrefix: "بداية الرقم",
  search: "بحث",
  searchPlaceholder: "البحث في الأرقام، المواقع...",

  // Filter Chips
  from: "من",
  to: "إلى",

  // Table Headers (if not already existing)
  callingNumber: "رقم المتصل",
  calledNumber: "رقم المستلم",
  callDuration: "مدة المكالمة",
  callLocation: "موقع المكالمة",
  callDate: "تاريخ المكالمة",
  siteId: "رقم الموقع",
  siteName: "اسم الموقع",

  // Table Pagination
  rowsPerPage: "عدد الصفوف في الصفحة",
  of: "من",

  // Sheet Comparison
  sheetsComparison: "مقارنة الشيتات",
  compareSheets: "مقارنة الشيتات",
  uploadFiles: "رفع الملفات",
  chooseFile: "اختر ملف",
  personName: "اسم الشخص/الشيت",
  fileFormat: "نوع الملف",
  addAnotherFile: "إضافة ملف آخر",
  startComparison: "بدء المقارنة",
  comparing: "جاري المقارنة...",
  needTwoFiles: "يرجى رفع ملفين على الأقل مع تسميتهما",

  // Results
  overlapSummary: "ملخص التطابق",
  commonContacts: "الأرقام المشتركة",
  overlapMatrix: "مصفوفة التطابق",
  interactiveNetwork: "الشبكة التفاعلية",

  // Statistics
  commonNumbers: "أرقام مشتركة",
  overlapPercentage: "نسبة التطابق",
  appearsIn: "يظهر في",
  totalSheets: "شيتات",

  // Network Graph
  showAllConnections: "جميع الاتصالات",
  showCommonOnly: "الاتصالات المشتركة فقط",
  showLabels: "إظهار الأسماء",
  sheetOwners: "أشخاص الشيتات",
  sharedContacts: "جهات اتصال مشتركة",
  individualContacts: "جهات اتصال فردية",
  selectedNodeInfo: "معلومات العقدة المحددة",
  nodeType: "النوع",
  sheetPerson: "شخص الشيت",
  contact: "جهة اتصال",

  // Matrix
  overlapMatrixDescription:
    "كل خلية تظهر نسبة الأرقام المشتركة من الشيت في الصف مقارنة بالشيت في العمود",
};

export default strings;
